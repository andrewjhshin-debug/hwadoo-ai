...
export const DAILY_CAP: Record<MeritSource, number> = {
  ...
  mandala: 5400, // 하루 두 장
  candle: 810,
  fortune: 105, // 패는 하루 한 장뿐이라 한 장 값이 곧 천장
  mung: 3150, // 십오 분. 더 앉아도 되지만 공덕은 여기까지
  // 형: 「하심도 할 때마다 주도록 해. 너무 많이는 말고」
  // 하루 네 번까지. 다섯 번째부터는 내려가도 값이 안 붙는다 —
  // 그때부터는 공덕이 아니라 그냥 내려가는 일이다.
  hasim: 2160,
  daily: 1620, // 오늘의 세 가지 — 하루 한 번뿐
};

/**
 * 하루 전체 천장 — **연꽃 한 송이(32,400 = 백팔 삼백 바퀴).**
 *
 * 형: 「모든 공덕 주는 기능을 재검토해서 **노가다 했을 때 반나절이면
 *      연꽃을 줄 만큼** 차도록 해」
 *
 * 6,480 이었다. 갈래를 고루 돌면 **한 시간이면** 천장에 닿았다 — 한 시간
 * 두드리고 하루치가 끝나니, 그 뒤로는 무엇을 해도 숫자가 안 움직였다.
 * 재화(연꽃)도 그만큼 가벼워졌다.
 *
 * 다섯 배로 올린다. 갈래마다의 값(MERIT_VALUE)은 **한 톨도 안 건드렸다** —
 * 한 배(7)는 여전히 한 배고, 한 편(54)은 여전히 한 편이다. 대신 천장까지
 * 가는 길이 다섯 배 길어진다. 값을 깎으면 한 번 한 일이 가벼워지지만,
 * 천장을 올리면 **한 일은 그대로고 갈 길만 멀어진다.** 그게 맞다.
 *
 * 얼마나 걸리나 — 갈래를 고루 돌아 **네다섯 시간**(반나절), 한 갈래만
 * 붙들면 못 채운다.
 *   백팔배 아홉 판(65분) 9,720 · 반야심경 스물다섯 편(75분) 8,100 ·
 *   호흡 40분 4,860 · 목탁·염주 5,400(30분) · 만다라 열 장 5,400 …
 * 갈래 천장을 다 더하면 57,255 = 이 수의 1.77배다. 예닐곱 갈래는 돌아야
 * 하고, 제일 큰 갈래(bow 9,720)도 30%를 못 넘는다.
 *
 * 연꽃은 **하루 한 송이까지만** 바꿀 수 있다(/api/lotus/exchange).
 * 천장이 곧 한 송이라 두 규칙이 서로 어긋날 일이 없다.
 */
export const DAILY_TOTAL_CAP = 32400;
...
export const LOTUS_PRICE = 32400;
...
export const RANKS = [
  ...
  { need: 32400, hanja: "沙", name: "사미", lotus: 3, say: "하루치를 꼬박 채웠어요" },
  { need: 226800, hanja: "首", name: "수좌", lotus: 9, say: "이레를 이었어요 — 앉는 일이 몸에 붙었어요" },
  { need: 972000, hanja: "禪", name: "선사", lotus: 27, say: "한 달. 흔들림이 눈에 띄게 줄었어요" },
  { need: 3888000, hanja: "薩", name: "보살", lotus: 81, say: "넉 달. 이제 남의 몫까지 돕니다" },
  { need: 11664000, hanja: "佛", name: "부처", lotus: 108, say: "" },
] as const;
