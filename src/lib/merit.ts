 ...
 * 갈래마다 하루에 쌓을 수 있는 공덕.
 *
 * 처음엔 갈래 천장을 바짝 조였더니, 하루치를 채우려면 열한 갈래를
 * 거의 다 돌아야 했다. 그건 수행이 아니라 숙제다. 그래서 천장을 열었다 —
 * 이제 예닐곱 갈래를 돌면 반나절에 하루치가 찬다.
 * 다 더하면 57,255 라 여전히 한 갈래만으로는 못 채운다.
 */
export const DAILY_CAP: Record<MeritSource, number> = {
  bow: 19440, // 백팔배 두 번 반(18분)
  ...
  inyeon: 600,
  sutra: 16200, // 반야심경 다섯 편(15분)
  moment: 8640,
  bowl: 4860,
  mandala: 10800, // 하루 두 장
  candle: 1620,
  fortune: 210, // 패는 하루 한 장뿐이라 한 장 값이 곧 천장
  mung: 6300, // 십오 분. 더 앉아도 되지만 공덕은 여기까지
  // 형: 「하심도 할 때마다 주도록 해. 너무 많이는 말고」
  // 하루 네 번까지. 다섯 번째부터는 내려가도 값이 안 붙는다 —
  // 그때부터는 공덕이 아니라 그냥 내려가는 일이다.
  hasim: 4320,
  daily: 3240, // 오늘의 세 가지 — 하루 한 번뿐
};

/**
 * 하루 전체 천장 — 43,200.
 *
 * 형: 「공덕 차는 게 너무 더디다. 반나절 또는 노가다 좀 하면 연꽃 주는
 *      걸로 해야 해. 너무 안 차면 그것도 그거대로 문제야. 맨 위 하루치
 *      공덕 차는 거 더 빨리 차게, 공덕 올리는 거 높여 보고. 전체 계급을
 *      위한 공덕 차는 것도 고려해서」
 *
 * ── 무엇을 어떻게 바꿨나 ──────────────────────────────────
 *  · **갈래 값을 정확히 두 배로** 올렸다(한 배 7→14, 한 편 54→108 …).
 *    두 배는 **모든 비(比)를 그대로 둔다** — 절과 경전과 멍의 저울이
 *    한 톨도 안 기운다. 값을 갈래마다 따로 만지면 저울이 무너진다.
 *  · 갈래 천장도 두 배로. 그래야 **횟수**가 그대로다(백팔배 두 번 반은
 *    여전히 두 번 반이다). 값만 올리고 천장을 두면 반만 하고 막힌다.
 *  · 하루 천장은 32,400 → **43,200**(1.33배). 값이 두 배인데 천장을
 *    그대로 두면 하루가 두 시간에 끝나 버린다. 조금만 올려 **차는 속도는
 *    빨라지고 하루의 무게는 남게** 한다.
 *
 * ── 얼마나 걸리나 ────────────────────────────────────────
 *  · 연꽃 한 송이(21,600 = 하루 천장의 절반) — 갈래를 고루 돌아 **한 시간
 *    반 남짓.** 「노가다 좀 하면」이 이 자리다(전에는 네다섯 시간).
 *  · 하루 천장 43,200 — 세 시간 안팎. 나머지 절반은 연꽃이 아니라
 *    **사다리**로 간다(하루 한 송이는 그대로다 — api/lotus/exchange).
 *  · 갈래 천장을 다 더하면 114,510 = 이 수의 2.65배. 한 갈래만으로는
 *    여전히 못 채운다(제일 큰 bow 19,440 도 45%).
 *
 * ── 사다리 ───────────────────────────────────────────────
 *  문턱은 늘 「하루 천장 × 날수」였다. 천장이 바뀌었으니 같이 옮긴다 —
 *  1일 43,200 · 7일 302,400 · 30일 1,296,000 · 120일 5,184,000 ·
 *  360일 15,552,000. **날수는 한 날도 안 바뀌었다.** 하루가 빨리 차니
 *  오르는 것도 그만큼 빨라진다(형: 「전체 계급도 고려해서」).
 *  육도(realm.ts REALMS)도 같은 수로 옮겼다 — 둘은 한 글자도 안 틀려야 한다.
 */
export const DAILY_TOTAL_CAP = 43200;
...
export const LOTUS_PRICE = 21600;
...
// 문턱은 realm.ts REALMS 와 **한 글자도 안 틀리게** 같아야 한다.
// rankByNeed 가 `r.need === need` 로 찾는다 — 하나라도 어긋나면 이름이
// 통째로 동자로 떨어진다. 육도를 고치면 여기도 같이 고친다.
// 연꽃(lotus)은 그 자리에 처음 올랐을 때 한 번 주는 몫이다.
export const RANKS = [
  { need: 0, hanja: "童", name: "동자", lotus: 0, say: "이제 막 산문에 들었어요" },
  { need: 43200, hanja: "沙", name: "사미", lotus: 3, say: "하루치를 꼬박 채웠어요" },
  { need: 302400, hanja: "首", name: "수좌", lotus: 9, say: "이레를 이었어요 — 앉는 일이 몸에 붙었어요" },
  { need: 1296000, hanja: "禪", name: "선사", lotus: 27, say: "한 달. 흔들림이 눈에 띄게 줄었어요" },
  { need: 5184000, hanja: "薩", name: "보살", lotus: 81, say: "넉 달. 이제 남의 몫까지 돕니다" },
  { need: 15552000, hanja: "佛", name: "부처", lotus: 108, say: "" },
] as const;

/**
 * 자리에 오를 때 받는 연꽃 — 형: 「계급 업글될 때마다 연꽃 많이 주는 걸로」.
 *
 * 하루치를 채워야 한 송이가 나오는 판에서, 자리 하나를 오르면
 * 사흘치(3) · 아흐레치(9) · 스무하루치(27) · 여든하루치(81) · 백여덟(108)이
 * 한꺼번에 들어온다. 오르는 데 걸린 날수에 견주면 크지 않은데(이레를
 * 걸어 아홉 송이) **한 번에** 들어오니 손에 잡힌다.
 * 3의 배수로 가다 마지막만 108 — 셈이 아니라 마무리다.
 */
export function rankLotus(need: number): number {
  return RANKS.find((r) => r.need === need)?.lotus ?? 0;
}
