...
export const MERIT_VALUE: Record<MeritSource, number> = {
  ...
  // 모멘트는 발품이 든다 — 절까지 가서 찍고 걸어야 한다.
  // 위치가 맞으면 걸 때 한 번 더 얹는다(moment.ts VERIFIED_BONUS).
  moment: 21,
...
export const DAILY_CAP: Record<MeritSource, number> = {
  bow: 972, // 백팔배 세 번
  moktak: 324, // 목탁 삼백스물네 번
  bead: 324, // 염주 세 바퀴
  breath: 189, // 호흡 아홉 판
  hwadu: 324, // 화두 셋
  temple: 216, // 절 네 곳
  gathering: 54, // 인연 글·댓글 여섯
  sutra: 972, // 반야심경 세 번
  moment: 63, // 하루 세 장 — 한 절에서 스무 장 올리는 판은 아니다
  bowl: 108, // 서른여섯 번 — 여운까지 들으면 십 분쯤이다
  daily: 54, // 오늘의 세 가지 — 하루 한 번뿐
};

/**
 * 하루 전체 천장 — **연꽃 한 송이와 같게 맞췄다.**
 * 한나절(두어 시간) 작정하고 하면 하루에 한 송이. 그 위로는 안 쌓인다.
 * 갈래 천장을 다 더하면 3,429 라 한 갈래만으로는 못 채운다 —
 * 목탁만 두드려서는 안 되고, 고루 해야 하루치가 찬다.
 */
export const DAILY_TOTAL_CAP = 3240;
...
// 수행 앱이 할 일이 아니다.
//
// 그래서 막는 자리를 바꿨다 — **하루에 쌓을 수 있는 공덕에 천장을** 두고,
// 환율을 그 천장과 **같게** 맞췄다. 백팔의 서른 배.
//   한나절(두어 시간) 작정하면 하루에 한 송이.
//   더 하고 싶어도 더 못 쌓으니, 갈아 넣어도 하루 한 송이가 끝이다.
// 우리 주머니가 비는 것도 아니다 — 연꽃은 결국 앱 안에서 쓰이고 사라진다.
