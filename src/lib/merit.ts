...
export const DAILY_CAP: Record<MeritSource, number> = {
  bow: 756, // 백팔배 한 번(7분)
  // 연타는 눌러 둔다. 목탁·염주를 합쳐도 432 — 하루치의 20%다.
  // 예전 천장(648·648)이면 두 엄지로 초당 대여섯 번씩 두드려 2분 반에
  // 하루치의 40%를 가져갈 수 있었다. TV 보면서 되는 일에 그 몫은 과하다.
  moktak: 216,
  bead: 216,
  breath: 378, // 열여덟 식(3분) — 한 판의 끝
  hwadu: 324, // 셋 — 하루 회향 횟수(GIVE_PER_DAY)와 같게
  temple: 324, // 세 곳
  gathering: 126, // 여섯 줄
  sutra: 648, // 반야심경 두 편(6분)
  moment: 324, // 그 자리에서 찍은 석 장(54×2)
  bowl: 189, // 아홉 번 — 한 번 치면 여운이 다 울 때까지 못 친다
  candle: 54, // 여섯 — 남의 초에 손 모으는 일은 덤이다
  daily: 108, // 오늘의 세 가지 — 하루 한 번뿐
};

/**
 * 하루 전체 천장 — **백팔을 스무 바퀴.**
 *
 * 3,240 에서 내렸다. 그 수는 분당 백 남짓으로 도는 판에서 서른 분이 넘게
 * 걸렸다. 서른 분은 수행이 아니라 숙제고, 숙제는 사흘이면 그만둔다.
 *   5~7분(가볍게)   600 남짓 · 28%
 *   15~17분(보통)  1,600 남짓 · 74%
 *   22~25분(작정)  2,160 · 천장 — 그 위로는 소리만 나고 공덕은 안 붙는다
 *
 * 뒤로 갈수록 싼 갈래가 먼저 떨어져 마지막 4분의 1이 앞의 절반만큼 시간을
 * 먹는다. 일부러 그렇게 뒀다 — 매일 100%를 찍고 나면 더 열 이유가 없어진다.
 * 4분의 3에서 「조금만 더 하면 되는데」 하고 덮는 자리가 오래 간다.
 *
 * 갈래 천장을 다 더하면 3,663 = 이 수의 1.7배. 하루치를 채우려면
 * 대여섯 갈래는 돌아야 한다. 제일 큰 갈래(bow 756)도 35%를 못 넘는다.
 */
export const DAILY_TOTAL_CAP = 2160;
...
export function giveMerit(to: string, n: number = GIVE_UNIT): MeritLedger | null {
  ...
  // 손에 쥔 몫에서 나간다. 모자라면 못 돌린다.
  //
  // 예전엔 한 톨도 안 줄었다. 대승의 셈이라 그렇게 뒀는데, 그러면 회향이
  // 선택이 아니라 하루 세 번 눌러 두는 단추가 된다 — 안 누를 이유가 없으니까.
  // 이제 쓴다. **자리(육도·계급)는 총합으로 매기므로 내려가지 않는다** —
  // 줄어드는 것은 연꽃으로 바꿀 수 있는 몫뿐이다.
  // 불을 나눠 줘도 내 불은 안 꺼지지만, 초는 닳는다. 그 말이 맞다.
  if (!spendMerit(give)) return null;
  const l = loadMerit();
  l.given += give;
  save(l);
  try {
