 ...
 * 그냥 볼 수 있는 수 — **하루 두 사람.**
 *
 * 형: 「하루에 최대 6개 보여지도록 할까? 한 번에 열 수 있고, 연꽃 한
 *      송이로 한 사람 더는 계속 열 수 있게? 그것도 제한은 두자,
 *      하루 4개까지 더?」
 *
 * 왜 하나가 아니라 둘인가 —
 *   하나면 열고 닫으면 끝이라 다시 올 까닭이 얕다. 그렇다고 셋을 주면
 *   연꽃 낼 일이 없어진다. **둘은 아쉬움이 남는 수**다 — 두 사람을 보고
 *   나면 「한 사람만 더」가 생긴다. 그 자리에 연꽃이 있다.
 *
 * 왜 무한이 아닌가 — 무한히 넘기는 판은 사람을 **고르는 물건**으로
