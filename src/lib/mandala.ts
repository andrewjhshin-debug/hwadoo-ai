...
// ── 색 ──────────────────────────────────────────────────────────
// 명도·채도의 폭을 넓게 편 네 벌 — 쨍한 원색(진채), 밝은 파스텔(담채),
// 중간 톤 전통 안료(단청), 깊이 가라앉은 색(현색). 이웃한 색끼리
// 확실히 구분되도록 색상(hue)을 번갈아 배열했다. 이름은 전통 색명.

export const PALETTE_GROUPS: { name: string; colors: string[] }[] = [
  {
    // 진채(眞彩) — 오방정색의 기세, 쨍한 원색
    name: "진채",
    colors: [
      "#E8402A", // 주홍(朱紅)
      "#F5B301", // 금황(金黃)
