...
//
// 처음엔 거꾸로 그렸다 — 밑이 둥글고 위가 뾰족했다. 실제 보리수 잎은 그 반대다:
//   **밑동이 하트(심장형)**로 두 갈래로 벌어지고, 위로 갈수록 좁아지다가
//   **끝이 꼬리처럼 길게 빠진다**(drip tip — 열대의 비를 흘려 보내는 생김).
// 그 두 가지가 보리수 잎을 한눈에 가르는 전부라, 그 둘만 그리고 나머지는 뺀다.
// 잎맥도 주맥 하나와 곁맥 한 쌍이면 족하다 — 더 그으면 14px 에서 뭉친다.
export function BodhiLeaf({ className = "w-5 h-5", stroke = "currentColor" }: IconProps) {
  return (
    ...
    >
      ...
      {/* 주맥 — 밑동 홈에서 끝까지 */}
      <path d="M12 16.9V5.4" opacity="0.55" />
      {/* 곁맥 한 쌍 */}
      <path d="M12 12.4 8.2 10.3M12 12.4l3.8-2.1" opacity="0.38" />
      {/* 잎자루 */}
      <path d="M12 16.9v4.7" opacity="0.7" />
    </svg>
  );
}
